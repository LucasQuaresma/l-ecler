...
export const Route = createFileRoute("/cursotoxina")({
  ...
  head: () => ({
    meta: [
      { title: "Curso Toxina Botulínica | 11 e 12 de setembro | L'ECLER Academy" },
      {
        name: "description",
        content:
          "Curso presencial de Toxina Botulínica em Harmonização Orofacial com a Dra. Cássia Blasques. Planejamento, anatomia, segurança e naturalidade na prática clínica.",
      },
      { property: "og:title", content: "Curso Toxina Botulínica | 11 e 12 de setembro" },
      {
        property: "og:description",
        content:
          "Imersão presencial para profissionais que desejam dominar protocolos de Toxina Botulínica com raciocínio clínico, segurança e previsibilidade.",
      },
      { property: "og:type", content: "website" },
    ],
    links: [{ rel: "canonical", href: "https://l-ecler.lovable.app/cursotoxina" }],
  }),
...
function CursoToxinaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      fetch("https://projeto01-n8n.gmxuno.easypanel.host/webhook/toxinasetembro", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      }).catch(() => {});

      navigate({ to: "/obrigadotoxina" });
    } catch (err) {
      console.error(err);
        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-white/70">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1.5 ring-1 ring-white/10">
              <Calendar className="h-4 w-4 text-[#c9a84c]" /> 11 e 12 de setembro
            </span>
            <span className="inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1.5 ring-1 ring-white/10">
          ...
          <div className="mt-6 rounded-2xl border border-[#c9a84c]/30 bg-[#c9a84c]/5 p-5">
            ...
            <p className="mt-1 font-display text-2xl">
              ...
              <span className="text-sm font-sans font-normal text-white/60">
                em Toxina Botulínica
              </span>
            </p>
            <p className="mt-2 text-sm text-white/70">
              Anatomia aplicada, planejamento de pontos, técnica de aplicação e conduta clínica para
              resultados naturais, seguros e previsíveis.
            </p>
          </div>

          <ul className="mt-6 space-y-2 text-sm text-white/80">
            {[
              "Protocolos faciais com foco em segurança e naturalidade",
              "Raciocínio anatômico para indicação e planejamento",
